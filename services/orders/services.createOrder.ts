...
export const servicesCreateOrder = {
    async createOrder({order_id, invoiceId, user_id, full_name, phone_number, city, department, email, comment, call, localCart}: 
        {order_id:number, invoiceId:string, user_id: number, full_name:string, phone_number: number, city:string, department:string, email:string | null, comment:string | null, call:string, localCart:any[]}) {
           try {
            const cart = new Cart(localCart)
            const total_price = await cart.getTotalCartPrice()
            await orderRepository.createOrder(order_id, invoiceId, user_id, full_name, String(phone_number), city, department, email, comment, call, total_price, "in procces", dateNow())          
           } catch (error) {
            console.error(error);            
           }
    },
    ...
    async updateStatus({order_id, invoiceId, order_status, force}: {order_id?:number, invoiceId?:string, order_status:string, force:boolean}) {
      try {
    ...
    async updateInvoiceId({order_id, invoiceId}: {order_id:number, invoiceId:string}) {
      try {
